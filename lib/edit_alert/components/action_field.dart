...
class _ActionListFieldState extends State<ActionListField> {
  ...
  void updateBloc(BuildContext context, List<TAction> actions) {
    context.read<EditAlertBloc>().add(ActionsChanged(actions));
  }
          ...
          child: TSelectedVanilla(
            ...
            onTapped: () async => showMaterialModalBottomSheet<Device?>(
              ...
              builder: (bContext) => BlocProvider.value(
                ...
                child: DeviceSheet(
                  selectedDeviceID: widget.device?.id,
              ...
              if (device != null) {
                if (widget.index >= 0 && widget.index < widget.actions.length) {
          ...
          child: TSelectedVanilla(
            ...
            onTapped: () async => showMaterialModalBottomSheet<Attribute?>(
              ...
              builder: (bContext) => AttributeSheet(
                ...
                attributes: widget.attributes
                    .where((att) => att.deviceID == _device?.id)
                    .toList(),
              ...
              if (attribute != null) {
                if (widget.index >= 0 && widget.index < widget.actions.length) {
                  final ac = widget.actions[widget.index];
                  final updatedAc = ac.copyWith(attributeID: attribute.id);
                  final actions = List<TAction>.from(_actions);
                  actions[widget.index] = updatedAc;
                  widget.onSaved(actions);
                  setState(() {
